<template>
  <div>
    <ProfileTabs :user="user" active="contributions" />

    <section class="section">
      <h3 class="title is-3">
        {{ user.display_name }} : {{ $t('PageUserProfile.contributions') }}
      </h3>
      ...
      >
        <b-pagination-button
          ...
          slot-scope="props"
          :page="props.page"
          :href="`/profile/${user.slug}/contributions?page=${props.page.number}`"
        >
          {{ props.page.number }}
        </b-pagination-button>
...
export default {
  ...
  async asyncData (context) {
    ...
    const contributions = await context.$axios.$get(`user/${context.route.params.slug}/contributions?page=${currentPage}`)

    return { user, contributions, currentPage }
  },
  methods: {
    onPageChange (page) {
    ...
    async fetchContributions () {
      ...
      await this.$axios.$get(`user/${this.user.slug}/contributions?page=${this.currentPage}`).then((res) => {
  ...
  head () {
    return {
      title: this.user.display_name + ' : ' + this.$t('PageUserProfile.contributions')
    }
  }
}
</script>
