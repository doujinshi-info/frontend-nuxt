<template>
  <div>
    <TagTabs :tag="tag" active="changelog" />
...
export default {
  ...
  async asyncData (context) {
    ...
    const contributions = await context.$axios.$get(`tag/${tagType}/${tagSlug}/changelog?page=${currentPage}`)

    return { tag, contributions, currentPage }
  },
  methods: {
    onPageChange (page) {
    ...
    async fetchContributions () {
      ...
      await this.$axios.$get(`tag/${this.tag.type.slug}/${this.tag.slug}/changelog?page=${this.currentPage}`).then((res) => {
  ...
  head () {
    return {
      title: this.getLocalizedName(this.tag.name) + ' : ' + this.$t('PageChangelog.title')
    }
  }
}
</script>
