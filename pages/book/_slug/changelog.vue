...
export default {
  ...
  async asyncData (context) {
    ...
    const book = await context.$axios.$get(`book/${slug}`)
    const contributions = await context.$axios.$get(`book/${slug}/changelog?page=${currentPage}`)

    return { book, contributions, currentPage }
  },
  methods: {
    onPageChange (page) {
    ...
    async fetchContributions () {
      ...
      await this.$axios.$get(`book/${this.book.slug}/changelog?page=${this.currentPage}`).then((res) => {
  ...
  head () {
    return {
      title: this.getLocalizedName(this.book.name) + ' : ' + this.$t('PageChangelog.title')
    }
  }
}
</script>
